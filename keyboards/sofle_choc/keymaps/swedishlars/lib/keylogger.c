...
char oled_key_code[KEYLOG_LENGTH] = {0};

// Store current active modifiers
char oled_key_mod[KEYLOG_LENGTH] = {0};

// Store current key description
char oled_key_desc[KEYLOG_LENGTH] = {0};
...
void add_keylog(uint16_t keycode, keyrecord_t *record) {
    ...
    if (sizeof(active_mods)) {
        snprintf(oled_key_mod, KEYLOG_LENGTH, active_mods);
    }
    else {
        snprintf(oled_key_mod, KEYLOG_LENGTH, OLED_EMPTY_LINE);
    }

    // store keycode TODO should have 10 char length!!
    snprintf(oled_key_code, KEYLOG_LENGTH, "kcd: %3d", keycode);

    // store key description
    snprintf(oled_key_desc, KEYLOG_LENGTH, get_keycode_desc(keycode, key_desc_mod));
}
